...
export class UserAuthController {
  async register(request: Request, response: Response) {
    ...
    response.cookie("isAuth", false, { maxAge: 86400000 * 2 });
  ...
  async twoFactorLogin(request: Request, response: Response) {
    const token = request.signedCookies.token;
    const userAuthCoreService = diContainer.get<IUserAuthCoreService>(
      DI_TYPES.UserAuthCoreService
    );

    const accessToken = await userAuthCoreService.twoFactorLogin({
      token,
